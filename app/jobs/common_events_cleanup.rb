  ...
  class CommonEventsCleanUp < VCAP::CloudController::Jobs::CCJob
    ...
    def self.delete_model_events_older_than(model, cutoff_age_in_days)
      old_events = if model.db.database_type == :mssql
                     model.dataset.where(Sequel.lit('CREATED_AT < DATEADD(DAY, -?, CURRENT_TIMESTAMP)', cutoff_age_in_days.to_i))
                   else
                     model.dataset.where(Sequel.lit("created_at < CURRENT_TIMESTAMP - INTERVAL '?' DAY", cutoff_age_in_days.to_i))
                   end
